...
(declare-function tramp-rpc--path-to-bin "tramp-rpc")
    ...
    (let* ((key (tramp-rpc-magit--get-cache-key vec directory))
      ...
      (dolist (entry results)
        (let* ((cmd-key (if (symbolp (car entry))
          ...
          ;; Admission failures are transient server load, not git results.
          ;; Returning them to the caller is useful, but caching them would
          ;; turn one overloaded refresh into a failure for the whole TTL.
  ...
  (when (and (file-remote-p directory)
    ...
    (with-parsed-tramp-file-name directory nil
      (let ((result (tramp-rpc--call v "ancestors.scan"
                                     `((directory . ,(tramp-rpc--path-to-bin localname))
