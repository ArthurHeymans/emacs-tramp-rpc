...
When STAT is not a symlink, also cache the corresponding follow/lstat spelling
because both variants return the same attributes."
  (let ((keys (list (tramp-rpc--file-stat-cache-key vec localname lstat))))
    (when (and stat (not (equal (alist-get 'type stat) "symlink")))
