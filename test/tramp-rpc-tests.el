    ...
    (unwind-protect
        (progn
          ...
          (let ((tramp-rpc--delivering-output t)
                (first-byte-delivered nil))
            (process-send-string p (substring utf8 0 1))
            ;; Service the first write before sending the remainder, without
            ;; relying on a wall-clock delay.
            (run-at-time 0 nil (lambda () (setq first-byte-delivered t)))
            (tramp-rpc-test--wait-for
             (lambda () first-byte-delivered) "first UTF-8 byte delivery" p)
