...
|----------------------------+-------+--------------------------------------------|
| ~tramp-rpc.el~             | 2626  | Core RPC communication & file handlers     |
| ~tramp-rpc-process.el~     | 1012  | Async process & PTY support                |
| ~tramp-rpc-magit.el~       | 1000  | Git/Magit/Projectile optimizations         |
| ~tramp-rpc-advice.el~      | 328   | Advice functions for process/VC integration|
...
- PTY processes with two modes:
  - *Direct SSH PTY* (default): Low-latency terminals via SSH ~-t~ flag
  - *RPC-based PTY* (fallback): PTY tunneled through RPC server
...
- *Server-side prefetch*: Single RPC replaces ~60+ git commands
- *Process-file cache*: Serves git commands from prefetched data
- *Ancestor scanning*: Fast project/VC root detection
- *File-exists cache*: TTL-based caching with automatic invalidation
- *Filesystem watching*: Inotify-based cache invalidation via server push
- *Projectile integration*: Optimized remote directory operations
