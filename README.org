...
- A remote host with either a published release architecture or a usable
  Rust/Cargo toolchain for a native source build
...
1. *Release/package installs*: try the checksum-verified release binary from GitHub Releases first.  If a matching source tree is present and the local and remote platforms match, attempt a local source build.  If a binary is unavailable for the platform or the remote is a different platform, TRAMP-RPC downloads the source archive for the installed release and tries to build it natively on the remote host.  For a supported platform with a usable remote Rust toolchain, interactive deployment asks whether to download the release binary or build remotely.
2. *Git checkout installs*: use the checked-out Rust sources when a source build is selected or needed.  With the default ~auto~ policy, existing source builds and source-tree keyed caches are reused.  When a new binary is needed on a supported remote platform with a usable Rust toolchain, interactive deployment asks whether to download a release binary, build remotely, or skip.  Run ~M-x tramp-rpc-deploy-install-binary~ to choose a release binary, a local or remote build of the checkout, or skip.  Strict ~build~ policy always uses checkout sources; ~release~ policy uses release-version paths and artifacts.

The default source-directory detection follows package-manager source links when they exist.  An archive/package installation normally contains only the Elisp files, so it has no local checkout to build.  In that case the tagged release source archive is downloaded temporarily for a remote build.  Set ~tramp-rpc-deploy-source-directory~ to a local checkout to use it, or to ~nil~ to disable all source builds.
...
Download from GitHub Releases
         ...
         │
         ▼
Build locally with Cargo when the platforms match
         │
         ├─ Success ────────────────► Cache locally, transfer to remote
         │
         ▼
Use a local checkout, or download the matching release source archive
         │
         ▼
Copy sources and build natively on remote with Cargo
         │
         ├─ Success ────────────────► Install remote binary
         │
         ▼
...
Other UNIX-like platforms MAY work using source builds.  When no release binary exists, automatic deployment attempts a native remote build.  For a supported platform with a usable remote Rust toolchain, interactive deployment asks whether to download the release binary or build remotely.

** Source and binary selection

The deployment source depends on how the Elisp package was installed:

| Installation                           | Release binary                                  | Source used for a remote build                                       |
|----------------------------------------+-------------------------------------------------+----------------------------------------------------------------------|
| Archive/package install                | GitHub release binary (for supported platforms) | Matching ~vVERSION~ source archive from GitHub, downloaded temporarily |
| Git checkout / recipe with ~auto~ policy | Existing source-keyed artifacts by default      | The detected local checkout                                          |
| Git checkout with ~release~ policy       | Release-version binary and cache paths          | The detected local checkout if a source fallback is needed           |
| Git checkout with ~build~ policy         | Not used as a fallback                          | The detected local checkout only                                     |

~tramp-rpc-deploy-prefer-build~ can be set to prefer a local source build before trying a release binary.

~tramp-rpc-deploy-source-directory~ is automatically inferred when possible, including source links created by checkout-oriented package managers.  A package archive generally has no Rust sources, and source builds will fall back to a release source archive.  Setting this option to ~nil~ disables both checkout and release-archive source builds.
...
1. Install Rust (see [[file:Cargo.toml][Cargo.toml]] for the current MSRV) and let tramp-rpc build (either locally or on the remote host for a source checkout):
   #+begin_src bash
...
1. *Rust Build* - Builds for multiple targets (including x86_64/aarch64 Linux/macOS) with format check
