    ...
    W: AsyncWrite + Unpin + Send + 'static,
{
    ...
    loop {
        if tasks.is_empty() {
            ...
            accept_frame(payload, &mut tasks, &writer, &admissions).await;
        } else {
            tokio::select! {
                Some(_) = tasks.join_next() => {}
                payload = frames.recv() => match payload {
                    Some(payload) => accept_frame(payload, &mut tasks, &writer, &admissions).await,
                    None => break,
                },
            }
        }
    ...
    W: AsyncWrite + Unpin + Send + 'static,
{
    ...
    let request = match decode_request(&payload) {
        Ok(request) => request,
        Err(response) => {
            write_response(writer, response.as_ref()).await;
            return;
        }
    };
    ...
    async fn test_process_run_stdin_write_failure_is_propagated() {
        let params = Value::Map(vec![
            ...
            (
                ...
                Value::Array(vec![
                    Value::String("-c".into()),
                    Value::String("exec 0<&-; sleep 0.1".into()),
                ]),
            ),
            (
    ...
    async fn test_connection_eof_kills_synchronous_process_groups() {
            ...
            .enumerate()
        {
            ...
            assert_eq!(
                ...
                Some(libc::ESRCH),
                "{method} child {child_pid} survived transport EOF"
            );
        }
    }
