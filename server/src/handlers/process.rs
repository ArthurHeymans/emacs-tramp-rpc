...
const MANAGED_CHILD_WAIT: std::time::Duration = std::time::Duration::from_secs(2);
...
impl PtyIoState {
    /// Publish cancellation without waiting for any async lock.
    fn cancel(&self) {
...
pub async fn write_pty(params: Value) -> HandlerResult {
    ...
    while offset < data.len() {
        ...
        // Cancellation publishes `closed` before its retained notification.
        // Rechecking after readiness prevents a write from starting after the
        // close transition.
        if io.is_closed() {
            return Err(RpcError::process_error(format!(
                "PTY write cancelled: {}",
...
mod tests {
    ...
    async fn cleanup_kills_pipe_descendants_after_status_reaps_direct_child() {
        ...
        panic!("descendant {descendant_pid} survived connection cleanup");
    }
