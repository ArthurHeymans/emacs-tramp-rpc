            ...
            let value = match cmd.spawn() {
                Ok(mut child) => {
                    ...
                    let write_stdin = async move {
                            ...
                            && let Some(mut stdin) = stdin.take()
                        {
                            stdin.write_all(&data).await.map_err(|error| {
                                std::io::Error::other(format!("Failed to write stdin: {error}"))
                            })
                        } else {
                            Ok(())
                        }
                    ...
                    let result = tokio::try_join!(
                        write_stdin,
                        super::process::read_sync_output(stdout, Arc::clone(&remaining)),
                        super::process::read_sync_output(stderr, remaining),
                        child.wait()
                    );
                    match result {
